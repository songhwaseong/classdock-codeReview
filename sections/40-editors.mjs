...
export default ({ manifest, helpers, rootDir }) => {
  ...
  return [
    ...
    mod('grid-selection.js', {
      title: 'grid-selection.js — 표 칸 고르기 셈 (MNGridSelection)',
      subtitle: 'DOM 을 모르는 순수 계산 — 스프레드시트 뷰어와 DB 결과 표가 함께 쓴다',
      summary:
        '"어느 칸이 선택됐는가" 와 "그것을 클립보드 글자로 어떻게 옮기고 되읽는가" 만 셈하는 모듈입니다. ' +
        '화면에 어떻게 칠할지는 쓰는 쪽이 정합니다 — 스프레드시트는 캔버스에, DB 결과 표는 <table> 에 그립니다. ' +
        '스프레드시트 뷰어 안에 있던 선택 셈을 DB 클라이언트가 같은 규칙을 써야 하게 되면서 떼어 낸 것이고, ' +
        'manifest 에 두 소비자와 함께 경계로 등록돼 있습니다.',
      usage: [
        {
          title: '칸 하나를 정수 하나로 눌러 담는다',
          body:
            '`행 * 열수 + 열` 로 키를 만들어 Set 하나에 담습니다. 그러면 흩어진 선택까지 같은 자료구조로 표현되어 ' +
            'Ctrl 로 더하기·빼기가 특별한 경우가 되지 않습니다. 사각 범위만 다루는 구조였다면 ' +
            '"떨어진 두 덩어리" 를 위해 목록을 따로 들고 다녀야 했을 자리를, 표현을 바꿔 없앤 것입니다.',
        },
        {
          title: '머리에서 시작한 드래그는 축을 유지한다',
          body:
            '행 머리를 눌러 끌기 시작했으면 포인터가 좁은 머리 띠를 벗어나도 계속 행 선택이어야 합니다. ' +
            'gridSelectionDragHitPoint 가 elementFromPoint 에 넘길 좌표를 시작한 축으로 투영해, ' +
            '행 선택은 Y 만·열 선택은 X 만 따라가게 합니다. 손이 조금 흔들렸다고 선택 종류가 바뀌지 않게 하는 처리입니다.',
        },
        {
          title: '고르지 않은 칸은 빈칸으로 내보낸다',
          body:
            '흩어진 선택도 사각형으로 감싸 옮기되, 고르지 않은 칸은 값 대신 빈칸을 넣습니다. ' +
            '거기에 실제 값을 채워 넣으면 붙여 넣은 쪽에서 "이건 내가 고른 게 아니다" 를 알 방법이 없기 때문입니다. ' +
            '복사는 화면을 옮기는 일이 아니라 고른 것을 옮기는 일이라는 구분입니다.',
        },
        {
          title: '내보내기와 되읽기를 같은 자리에 둔다',
          body:
            'gridSelectionToText 가 TSV 규칙으로 감싸고(탭·줄바꿈·따옴표가 든 값은 큰따옴표로, 안의 따옴표는 두 번) ' +
            'gridClipboardTable 이 그것을 되읽습니다. 두 표가 서로 복사·붙여넣기를 하므로 규칙이 갈라지면 ' +
            '한쪽에서 복사한 것이 다른 쪽에서 어긋납니다. 짝이 되는 함수를 떨어뜨려 두지 않은 이유가 주석에 적혀 있습니다.',
        },
        {
          title: '따옴표는 칸의 첫 글자일 때만 인용 부호',
          body:
            '중간에 나온 따옴표까지 문법으로 삼으면 외부 앱에서 온 `He said "hi"` 같은 평문이 조용히 변형됩니다. ' +
            '엑셀·구글시트에서 복사한 글자를 받는 파서라 "우리가 쓴 형식" 보다 "남이 준 형식" 을 견디는 쪽으로 규칙을 좁혔습니다.',
        },
        {
          title: '붙여넣기가 표를 늘리지 않는다',
          body:
            'gridPastePlan 은 표 밖으로 넘친 행·열을 버리고 그 수만 돌려줍니다. ' +
            '결과 표의 행은 서버의 행이지 시트의 칸이 아니라 늘릴 수 없기 때문입니다 — ' +
            '두 소비자의 성질이 다른 자리를 순수 계산 쪽에서 미리 갈라 두고, 넘친 사실은 화면이 알리게 넘깁니다. ' +
            '칸 하나만 복사했으면 고른 칸 전부를 그 값으로 채우는 엑셀 규칙도 여기 들어 있습니다.',
        },
      ],
      features: [
        { title: '선택 셈', body: 'gridSelectionRangeKeys · RangeCovered · CombineKeys(replace·add·subtract) · BoundsFromKeys · RangeBetween.' },
        { title: '이미 덮인 범위는 빼기', body: 'RangeCovered 로 "이미 다 고른 범위를 Ctrl 로 끌면 빼기" 를 판정합니다.' },
        { title: 'contiguous 표시', body: 'BoundsFromKeys 가 키 수와 사각 넓이를 비교해 선택이 연속인지 함께 돌려줍니다 — 정렬·내보내기에서 갈리는 값입니다.' },
        { title: '클립보드', body: 'gridSelectionToText(내보내기) · gridClipboardTable(되읽기) · gridPastePlan(자리 맞추기).' },
      ],
      files: [
        { path: 'tests/db-client.test.js', label: 'db-client.test.js', description: '범위·더하기/빼기·경계·TSV 옮기기를 두 표가 같은 모듈에서 얻는지 검사' },
      ],
      notes: [
        {
          type: 'good',
          label: 'Good',
          body:
            '두 번째 소비자가 생기는 시점에 뗐습니다. 스프레드시트 뷰어 안에 있던 선택 셈을 그대로 복사해 ' +
            'DB 결과 표에 붙였다면, 그 뒤로 "엑셀에서 복사한 것이 한쪽에서만 제대로 붙는" 종류의 어긋남이 생겼을 자리입니다. ' +
            '떼면서 moduleBoundaries 에 등록해 check-source.js 의 경계 검사(두 소비자보다 먼저 로드되는지)까지 받게 했습니다.',
        },
        {
          type: 'good',
          label: 'Good',
          body:
            'DOM 을 한 줄도 만지지 않아 그대로 단위 테스트됩니다. ' +
            '표 선택은 손으로 확인하기 가장 번거로운 종류의 UI 인데(끌기·Ctrl·Shift·머리 클릭의 조합), ' +
            '그 조합을 전부 순수 함수로 내려놓아 브라우저 없이 확인할 수 있게 했습니다.',
        },
        {
          type: 'risk',
          label: 'Risk',
          body:
            '키가 `행 * 열수 + 열` 정수라 열 수(maxCols)가 바뀌면 담아 둔 키 전체가 다른 칸을 가리킵니다. ' +
            '함수마다 maxCols 를 인자로 받아 그때그때 푸는 구조라 모듈 자신은 안전하지만, ' +
            '쓰는 쪽이 열이 바뀐 뒤에도 옛 Set 을 들고 있으면 조용히 엉뚱한 칸이 선택된 것처럼 보입니다. ' +
            '표를 다시 그릴 때 선택을 비우는 책임이 전적으로 소비자에게 있습니다.',
        },
        {
          type: 'info',
          label: 'Info',
          body:
            '이 모듈의 테스트는 자기 이름의 파일이 아니라 tests/db-client.test.js 안에 있습니다. ' +
            '떼어 낸 계기가 DB 결과 표였기 때문인데, 소비자가 둘인 공용 모듈의 검사가 한쪽 소비자의 테스트 파일에 얹혀 있는 상태입니다.',
        },
      ],
    }),
