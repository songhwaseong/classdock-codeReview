...
const categoryForPath = (filePath) => {
  ...
  // timeline(연대표)은 map-viewer 와 같은 문서 종류다. 파일 이름에 viewer 가 없다는 이유로
  // '앱 코어' 로 떨어져 core.js·documents.js 와 25칸을 다투고 있었다 — 몫을 나눠 둔 뜻이 사라진다.
  // subway·jeju-bus 는 지도 문서에 얹는 실시간 층이라 map-viewer 와 같은 영역이다(이름에 viewer 가 없다).
  if (/document|spreadsheet|excel|word|pdf|ppt|odt|hwp|viewer|timeline|subway|jeju-bus/.test(value)) return '문서·편집';
  // java-editor.js 가 아래 editor 규칙에 먼저 걸리지 않도록 자바를 편집기보다 앞에서 본다.
  // 자바는 따로 몫을 두지 않고 같은 "브라우저 밖 언어 실행" 칸을 JavaScript 와 나눈다. 예제 갤러리도
  // 파이썬·자바 공용이라 여기 둔다(python 규칙은 위에서 이미 python-snippets.js 를 가져갔다).
  if (/javascript|js-runtime|js-librar|(?:^|\/)java-|snippet-gallery/.test(value)) return 'JavaScript·Java 실행';
  if (/editor|monaco|codemirror/.test(value)) return '편집기';
  // terminal 은 위의 python 검사를 지난 뒤이므로 python-terminal.js 는 여기 오지 않는다(= 원격 터미널만).
  if (/learning|whiteboard|quiz|flashcard|mindmap|chart|media|image|terminal/.test(value)) return '학습 도구';
  return '앱 코어';
};
